...
The workbench is a layout contribution on the existing web client, not a new Electron package family. [`ui-layout`](../../../../packages/client/ui-layout/README.md) owns a three-column Agent frame (`sidebar` | `conversation` | `details`) with switchable sidebar occupants (`sidebar.agent`, `sidebar.scm`). There is no activity bar and no empty IDE seats. SCM opens from the Agent sidebar foot. SCM file diffs occupy `details.scm` and reuse [`DiffBlock`](../../../../packages/client/ui-primitives/README.md). `sidebarView`, `detailsView`, and `scmSelection` live in the layout store.
