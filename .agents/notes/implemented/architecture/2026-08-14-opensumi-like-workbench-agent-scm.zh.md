...
桌面和 web 产品共享同一套工作台。SCM 从 Agent 侧栏底部打开；没有活动栏。第一期 SCM 停在本地 index 与提交；远程、合并和 blame 仍由 Agent／`bash` 完成。特权 `git.*` 仅限回环，远程浏览器不能驱动宿主的 Git 二进制。
