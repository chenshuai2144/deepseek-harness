...
The [web configuration plane](../architecture/2026-07-30-web-config-plane.md) makes provider settings and credentials live-editable, but a user can land on the empty conversation Hero or restore an existing conversation without an actionable explanation when the shipped `deepseek-official` route has no credential. The Models page can repair that state, yet requiring the user to discover it weakens onboarding and credential recovery. A prompt must not confuse a missing credential with a missing adapter: the browser can store a value for an existing credential reference, but it cannot dynamically mount the `llm-deepseek` Cordis plugin.
...
**The settings shell contributes ordering and current-view state, not provider policy.** `ui-settings` declares a root-scoped `settings.onboarding` list slot and mounts one ordered step at a time after the Session list is ready. The active registrant receives `firstRun`, `complete()`, and a private `openSection(id)` callback; completion transfers ownership to the next entry. The versioned welcome notice completes without loading when `firstRun` is false, while the DeepSeek step evaluates provider readiness on both the first-run surface and restored nonblank sessions. `ui-settings-models` registers both steps and its Models section through `slots.inject()`, so every contribution follows one client Cordis plugin's lifecycle and the dialogs cannot stack. Their common presentation is owned by the [shared-modal onboarding decision](2026-08-13-shared-modal-product-onboarding.md).
