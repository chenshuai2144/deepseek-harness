...
| `desktop` | forwarded to the Host; the first-period shell has no app flags of its own |
...
`dsh desktop` launches the Electron Agent GUI from a repository checkout. Config dumps (`--dump-config`, `--dump-default-config`) resolve as `--profile desktop`. Otherwise the launcher spawns the `apps/desktop` Electron main process and does not listen on a port. Arguments after the launcher flags are forwarded to the Host as `cmdlineArgs`. Electron lives only in `apps/desktop` and is not part of the published `@deepseek-ai/dsh` install. The frontend dist must already be built (`pnpm run build`).

```sh
pnpm dsh desktop
pnpm dsh desktop --dump-config
```
