...
| `desktop` | 转发给 Host；第一期外壳没有自己的应用 flag |
...
`dsh desktop` 从仓库检出启动 Electron Agent GUI。配置 dump（`--dump-config`、`--dump-default-config`）按 `--profile desktop` 解析。否则启动器拉起 `apps/desktop` 的 Electron 主进程，不监听端口。启动器 flag 之后的参数作为 `cmdlineArgs` 转发给 Host。`electron` 只放在 `apps/desktop`，不进入已发布的 `@deepseek-ai/dsh` 安装包。前端 dist 需已构建（`pnpm run build`）。
