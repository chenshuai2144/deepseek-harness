...
describe('web e2e: Code Mode round renders nested sub-calls', () => {
  ...
  it.skipIf(MODE === 'record')('a bash sub-row click leaves the workspace home occupant', async () => {
    onTestFailed(() => saveFailureShot(page, 'web-e2e-code-mode-details'))
    const nest = page.locator('[data-subcalls]').first()
    const frame = page.locator('[style*="grid-template-columns"]').first()
    expect(await frame.getAttribute('data-details-view')).toBe('home')
    expect(await frame.getAttribute('data-details-collapsed')).toBeNull()
    await nest.locator('[data-sample="bash"]').first().click()
    // Tool rows do not drive layout geometry; the Session keeps the workspace home.
    await expect.poll(() => frame.getAttribute('data-details-view'), { timeout: 5_000 }).toBe('home')
  })
