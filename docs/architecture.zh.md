...
[`dsh-base`](../packages/bundle/base/README.md) 是每个 profile 的第一层：模型适配器、工具、持久化、沙箱与审批策略、设置、凭据、遥测。[`dsh-web-app`](../packages/bundle/web-app/README.md) 增加浏览器应用；[`dsh-headless`](../packages/bundle/headless/README.md) 增加一次性运行器，且完全不带服务器；[`dsh-desktop-app`](../packages/bundle/desktop-app/README.md) 叠在 web 花名册上、去掉 HTTP，并在 Electron 中承载同一套 UI。三栏 Agent 工作台和 SCM 面板是那套共享客户端上的 layout 贡献，不是第二套 Electron 包族。
...
| 添加工作台 Simple Browser | 注册 `ui-browser`；模型继续使用 web 工具 |
