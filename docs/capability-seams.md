...
| `ctx.git` | `seam` | [`git`](../packages/git/git) | [`git-local`](../packages/git/git-local) | `apiproxy` | - | Serves the human SCM panel through privileged Host RPC; the model continues to use bash for Git operations. |
