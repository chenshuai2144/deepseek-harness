...
flowchart LR
  ...
  pkg_git["git"]
  svc_git["ctx.git<br/>Workspace Git provider seam"]
...
| `ctx.git` | `seam` | [`git`](../packages/git/git) | [`git-local`](../packages/git/git-local) | `apiproxy` | - | 通过特权 Host RPC 服务面向人的 SCM 面板；模型继续使用 bash 执行 Git 操作。 |
