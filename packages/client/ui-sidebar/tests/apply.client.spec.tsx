...
async function bench(declare = true) {
  ...
  if (declare) {
    slots.register(
      { name: 'root', children: { 'sidebar.agent': { kind: 'single', scope: 'root' } } } as never,
      () => null,
    )
  }
...
describe('ui-sidebar apply', () => {
  ...
  it('registers the shell and declares its child seats', async () => {
    ...
    expect(b.slots.entries('sidebar.agent')).toHaveLength(1)
  ...
  it('removes the entry and child declaration on teardown', async () => {
    ...
    await fiber.await()
    await fiber.dispose()
    expect(b.slots.entries('sidebar.agent')).toHaveLength(0)
