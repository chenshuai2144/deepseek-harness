...
`fs.listDir` 与 `fs.readText` 为 File 栏在一个会话工作区内列举并预览文件。客户端把绝对工作区根作为 `cwd` 提交；网关经 `ctx.fs` 解析，拒绝相对根或越出该根的路径，并把预览限制在 1 MiB UTF-8 字节，超出时带 `truncated: true`。这两个方法与 `git.*` 一样钉在回环。未组合 `ctx.fs` 的部署返回 `fs-unavailable`。
